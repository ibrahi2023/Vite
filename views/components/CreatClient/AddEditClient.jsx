...
export default function AddEditClient() {
    ...
    console.log(user)
    const [divData, setdivData] = useState({CTS: '', nom: '',prenom:'',date_naissansse:'',sexe:"",wilaya:"",commune:"", n_telephone:"",groupe_sanguin:"",phenotype:"",date_don:"",inscrip_registre:"",matricule:"",annee:"",month:"",_id:""});
    ...
    const handleChange3 = () => {
        ...
        if(validationUtility.isEmpty(divData.CTS)){
            ...
            setResponse(data);
            setTimeout(() => {
                setdivData(prevState => ({...prevState, ["_id"]: data.data._id}));
            }, 200); 
            } catch (error) {
 ...
 const  handleAnnule= async()=> {
    setTimeout(() => {
        navigate("/PageMenu",{state:{user:user,title:"Page Menu"}})
       }, 200); 
    }  
    const  GetClient=async()=> {
        try {
            ...
            let arr=[],ins="";
               ...
               if(result?.length>0){
               for(var i in result){
                arr[i]=result[i].inscrip_registre;
...
<div   className="flex flex-row w-3/4 shadow-md bg-blue-50 px-8 pt-6 pb-8 mb-4 my-2 ml-40 mt-10">
   ...
   { (ResponseData) ? (<div>
    ...
    <div className="-mx-3 md:flex mt-6 mb-1">
       ...
       {!ResponseData? (<button onClick={handleSubmit} type="button" style={{width: "240px"}}className="text-center ml-0 px-2 py-3 text-purple-600 hover:text-purple-700 focus:text-purple-700 hover:bg-purple-100 focus:bg-purple-100 border border-purple-600 rounded-md mb-3">
        {loading?(<span>Ajouter Client Don..</span>):(<span>Ajouter Client Dondd</span>)}
        </button>):( <button onClick={handleSubmit2} type="button" style={{width: "240px"}}className="text-center ml-0 px-2 py-3 text-yellow-600 hover:text-yellow-700 focus:text-yellow-700 hover:bg-yellow-100 focus:bg-yellow-100 border border-yellow-600 rounded-md mb-3">
